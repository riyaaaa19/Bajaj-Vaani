...
const Chatbot = ({ token }) => {
  ...
  const [fileFlag, setFileFlag] = useState(""); // document flag
  ...
  const sendMessage = async () => {
    ...
    // Clear input but keep file flag until bot responds
    setQuery("");
    setLoading(true);
    ...
    try {
      ...
      if (files.length > 0) {
        ...
        if (query.trim()) formData.append("query", query.trim()); // send query with files
      ...
      // Clear files and document flag after bot responds
      setFiles([]);
      setFileFlag("");
    } catch (err) {
      setMessages((prev) => [
