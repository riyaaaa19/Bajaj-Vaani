...
const Chatbot = ({ token }) => {
  ...
  const [flagKey, setFlagKey] = useState(0); // internal key to force re-render

  const messagesEndRef = useRef(null);

  useEffect(() => {
    messagesEndRef.current?.scrollIntoView({ behavior: "smooth" });
  }, [messages, fileFlag, flagKey]);
  ...
  const handleFileChange = (e) => {
    ...
    if (selectedFiles.length > 0) {
      // Increment key to force re-render of flag even if same file
  ...
  const sendMessage = async () => {
    ...
    setQuery("");
    setFileFlag(""); // hide temporary flag after sending
    setLoading(true);
  ...
  return (
    ...
    >
      ...
      {fileFlag && (
        <div
          key={flagKey} // forces React to re-render even if same file
          style={{
            padding: "8px 16px",
            background: "#e6f2ff",
            color: "#333",
